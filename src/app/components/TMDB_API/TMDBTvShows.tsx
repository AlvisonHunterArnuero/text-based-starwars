...
import { fetchTMDBTvShows } from '@/app/actions/tmdbActions';
...
const TMDBTvShows: FC = () => {
  ...
  useEffect(() => {
    const getTMDBTvShows = async () => {
      try {
        setLoading(true);
        const results = await fetchTMDBTvShows(currentPage);
